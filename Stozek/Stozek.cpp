// Stozek.cpp : Defines the entry point for the console application.
//
...
void RenderScene(void)
{
	...
	for (angle = 0.0f; angle < (2.5*GL_PI); angle += (GL_PI / 2.5f))
	{
	...
	for (angle = 0.0f; angle < (2.5f*GL_PI); angle += (GL_PI / 2.5f))
	{
		// Wyliczenie wspó³rzêdnych x i y kolejnego wierzcho³ka
		x = 50.0f*sin(angle);
		y = 50.0f*cos(angle);
...
int main(int argc, char* argv[])
{
	...
	glutAddMenuEntry("Shade model Smooth or Flat", 1);
